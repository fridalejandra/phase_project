...
## Arange months up to Feb to July ##
def is_months(month):
...
def Data_Array(breakup,year):
    ...
    filename = f"yr{file_yr}_A_5d_15p.nc"
    temp_breakup.to_netcdf(f'/Users/fridaperez/Developer/repos/phase_project/Advance/netcdf/{filename}')

for year in list(range(2012,2024)):
    temp_da = sic_xr.sel(time=sic_xr.time.dt.year.isin([year]))
    freezeup = continuous_meet(temp_da >= 15, window_size=5, dim='time')
    Data_Array(freezeup,year)
