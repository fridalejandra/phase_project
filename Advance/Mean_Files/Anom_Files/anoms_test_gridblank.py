...
# Set up the figure and grid, including an extra empty subplot
fig, axes = plt.subplots(num_rows, num_cols, figsize=(16, 9))
plt.subplots_adjust(wspace=0.4, hspace=0.3)

# Remove the last subplot from the grid
if num_years % num_cols != 0:
    fig.delaxes(axes[num_years // num_cols, num_years % num_cols])
...
for i, year in enumerate(years):
    ...
    if os.path.exists(file_path):
        ...
        li = map.imshow(ice, extent=[x.min(), x.max(), y.min(), y.max()], cmap='RdBu_r', vmin=-50, vmax=50)
